...
/** True when `code` is one of `list` (case-insensitive; non-arrays → false). */
function codeIn(list, code) {
  if (!Array.isArray(list)) return false;
  const want = String(code).toUpperCase();
  return list.some((c) => String(c).toUpperCase() === want);
}

/**
 * Operations for the KIT code node. Requires input.triggeringDiscountCode.
 * v14.3 grant rule (case-insensitive):
 *  - a LADDER code (one of ss.tiers[].c) grants ONLY when it equals the code
 *    of the tier the cart qualifies for (as before);
 *  - an ALIAS code (one of ss.alias — legacy KIT codes kept working) grants
 *    the qualifying tier, whatever it is;
...
export function computeKit(cfg, input) {
  ...
  if (!tier) return [];
  // Codes are case-insensitive on Shopify's side (a buyer typing "kit2" gets code "KIT2").
  const isLadder = codeIn((Array.isArray(ss.tiers) ? ss.tiers : []).map((t) => t && t.c), code);
  if (isLadder) {
    if (String(tier.c).toUpperCase() !== String(code).toUpperCase()) return [];
  } else if (!codeIn(ss.alias, code)) {
    return [];
  }
