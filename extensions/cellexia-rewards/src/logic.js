...
/**
 * v18: the cluster serving `country`, or the catch-all. Total by
 * construction — the server guarantees exactly one `rest` cluster — so the
 * Function, the storefront and the admin all resolve a country the same way.
 */
export function clusterFor(gt, country) {
...
/**
 * v18 per-tier thresholds in the cart currency, for ONE cluster: the
 * COUNTRY's own amount (bc[country].a[i]) when it is denominated in the cart
 * currency and is > 0 (an empty or 0 slot means "not overridden"), else the
 * cluster's EUR amount × rate. Mirrors rwGiftThresholdCents in the theme.
 */
export function giftAmounts(gt, country, currency, rate) {
...
export function computeGifts(cfg, input) {
  ...
  // v18: gifts are decided by COUNTRY, not market — a cluster is a group of
  // countries with its own ladder, and the buyer country is already in the
  // Function input, so no schema change was needed here.
