 ...
 * code the buyer removed; v14.3: when a ladder code and a shopper-typed
 * legacy ALIAS (`rewards.setSavings.aliasCodes`) sit together, the ladder
 * code is removed once so the alias wins (never the reverse, aliases are
 * never attached). Both are best effort (every error swallowed),
...
/** Storage key: our ladder code was removed once in favour of a typed alias (v14.3). */
const KIT_ALIAS_YIELDED_KEY = 'cellexia_kit_alias_yielded';
...
interface RewardsConfig {
  ...
  giftTiersOn: boolean;
  tiers: RewardsTier[];
  /** v14.3 legacy alias codes (rewards.setSavings.aliasCodes, server-computed:
   *  LEGACY_KIT_CODES minus the ladder), upper-cased. A shopper-typed alias
   *  grants the qualifying tier; the safety net never attaches one. */
  aliasCodes: string[];
...
/** v14.3: alias list minus the ladder codes, upper-cased, deduped. */
function parseAliasCodes(raw: unknown, tiers: RewardsTier[]): string[] {
  if (!Array.isArray(raw)) return [];
...
function resolveRewards(root: Record<string, unknown> | undefined): RewardsConfig {
  ...
  return {
    ...
    tiers,
    aliasCodes: parseAliasCodes(setSavings.aliasCodes, tiers),
...
function qualifyingTier(tiers: RewardsTier[], distinct: number): RewardsTier | null {
...
function RewardsSafetyNet() {
  ...
  // (v14.3: still ONLY when no code at all — a typed alias, applicable or
  // not, is a code, so nothing is attached next to it; aliases themselves
  // are never attached).
  const tiers =
  ...
  const desiredCode = qualifyingTier(tiers, distinctEligible)?.code ?? null;
  ...
  // v14.3 (a): a ladder code AND a shopper-typed legacy alias together —
  // the alias wins, our ladder code goes (at most ONE KIT-family code).
  ...
  const aliasPresent = useMemo(() => {
    if (!Array.isArray(discountCodes) || rewards.aliasCodes.length === 0) return false;
    const aliases = new Set(rewards.aliasCodes);
    return discountCodes.some(
      (entry) => typeof entry?.code === 'string' && aliases.has(entry.code.toUpperCase()),
    );
  }, [discountCodes, rewards.aliasCodes]);
  const ladderToYield = aliasPresent && ladderCodesPresent.length > 0 ? ladderCodesPresent[0] : null;
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        // Once per checkout session (storage memory): remove OUR ladder code
        // so the shopper's typed alias is the only KIT-family code; the alias
        // and any foreign code are never touched. Storage failures fail
        // closed (no removal).
        let stored: unknown;
        try {
          stored = await storage.read(KIT_ALIAS_YIELDED_KEY);
        } catch {
          return;
        }
        if (stored != null) return;
        try {
          await storage.write(KIT_ALIAS_YIELDED_KEY, ladderToYield);
        } catch {
          return;
        }
