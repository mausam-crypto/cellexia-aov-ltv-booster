...
(function () {
  ...
  function track(feature, type, meta) {
    ...
    try {
      ...
      // v22: a free-text dimension the Event row already carries. Used to
      // split an impression by which URL-gated pieces the visitor saw, so
      // the gated and ungated arms are comparable.
      if (typeof meta === 'string' && meta) payload.meta = meta;
  ...
  function cxTpScale(node, tp) {
    // v30 merchant size control (trustpilot.scale, shared by the PDP strip
    // and the proof block's rating row): both rows' metrics are authored
    // as calc(var(--cxtp, 1) * Npx) in the stylesheet (the v28 award-strip
    // technique), so ONE inline custom property scales stars, score, count
    // and the wordmark together. The island carries `s` — and this writes
    // the property — ONLY when the merchant asked for MORE than 100%: a
    // pre-v30 mirror (no member), junk, 100 or anything below leaves the
    // attribute off entirely, byte-identical DOM.
    var rs = tp ? Number(tp.s) : NaN;
    if (isFinite(rs) && rs > 100) {
      if (rs > 200) rs = 200;
      node.setAttribute('style', '--cxtp:' + (Math.round(rs) / 100));
    }
  }
  ...
  function bbpRatingRow(conf) {
    ...
    // v30: size rides the SHARED trustpilot.scale via the tp member's `s`
    // (cxTpScale) — the strip and this row always scale together.
    cxTpScale(row, tp);
  ...
  function bbpSealNode(conf) {
    ...
    // v22: the seal can carry its own URL parameter gate. PREVIEW always
    // renders it so the merchant can check the design in the Preview Center
    // (track() already suppresses every beacon there).
  ...
  function bbpResearchNode(d, conf) {
    ...
    // v22: the research band's own gate, independent of the seal's — either
    // piece can be link-only while the other stays visible to everyone.
    var researchGated = !!(research && research.gate) && !PREVIEW && !cxGateOpen(research.gate, Date.now());
    var items = research && research.enabled !== false && !researchGated && Object.prototype.toString.call(research.institutions) === '[object Array]'
  ...
  function bbpGateMeta(conf, research, award) {
    ...
    // arm (gates configured, this visitor saw none) and "g:br,bs" is the
    // treated arm. Both are needed for the comparison to mean anything.
    try {
      var ids = [];
      var gated = false;
      if (conf.research && conf.research.gate) {
        gated = true;
        if (research && research.querySelector('.cx-bbp-research__col')) ids.push(conf.research.gate);
      }
  ...
  // "2 Strzykawki", "5 Strzykawek", Arabic duals, Finnish partitives,
  // Hungarian numeral-singular, Japanese counters). CURATED, not machine
  // translated: plural tables are grammar, not prose (AZ_SHIPS_FORMS
  // rationale). Missing locale/unit/category falls back through
  // other -> one -> the variant-title label, never a broken string.
  ...
  function qselLabel(title) {
    ...
    // save chip is the truth this design shows instead. (The translated
    // option values use an EN DASH: "2 Tubes – 15% de réduction",
    // verified live 2026-09-19 — the class covers - – —.)
    var t = typeof title === 'string' ? title : '';
    var cut = t.search(/\s+[-–—]\s+/);
    return cut > 0 ? t.slice(0, cut) : t;
  }

  function qselPlural(l, n) {
    // v27: CLDR category for the unit catalog, exact for counts 1..6 (the
    // variant cap). Locales absent here are one/other.
    var base = (typeof l === 'string' ? l : '').split('-')[0].toLowerCase();
    if (base === 'pl') return n === 1 ? 'one' : n >= 2 && n <= 4 ? 'few' : 'many';
    if (base === 'ro') return n === 1 ? 'one' : 'few';
    if (base === 'ar') return n === 1 ? 'one' : n === 2 ? 'two' : 'few';
    if (base === 'hu' || base === 'ja') return 'other';
    return n === 1 ? 'one' : 'other';
  }

  function qselUnitLabel(d, v) {
    // v27: a product mapped to a unit type composes "{n} <unit>" with the
    // curated native plural form; anything unresolvable (no mapping, no
    // parseable count, unknown unit or locale gap) returns null and the
    // card keeps the variant-title label — never a broken string.
    var u = d && typeof d.u === 'string' ? d.u : '';
    if (!u) return null;
    var q = qselQty(v.t);
    if (q < 1) return null;
    var pack = CX_QSEL_UNITS[qselLocale(d)];
    var forms = pack && pack[u];
    if (!forms) return null;
    var form = forms[qselPlural(d.l, q)] || forms.other || forms.one;
    if (typeof form !== 'string' || !form) return null;
    return form.split('{n}').join(String(q));
  }
  ...
  function qselCard(d, v, base, badge) {
    ...
    name.textContent = qselUnitLabel(d, v) || qselLabel(v.t);
  ...
  function boot() {
    // v22: capture the parameter gate FIRST — synchronously, before the
    // preview round trip and before init() paints, so a visitor landing
    // straight on a product page through a tagged link sees the gated piece
    // on the very first view instead of after a jump.
    cxGateBoot(Date.now());
