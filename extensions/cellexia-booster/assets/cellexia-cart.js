...
(function () {
  ...
  // v13 CART-ONLY (not a twin — the cartExcludedAny precedent): one-shot
  // heal of the cart's _cx_us_state attribute toward the CURRENT explicit
  // choice. Covers carts created AFTER the choice was stored (post-order
  // carts, cleared cookies) and choices cleared while no cart write could
  // ride. The cart island's usAttr member carries the server-rendered
  // attribute, so the verdict costs no read request; at most ONE write
  // fires per page view, and only on a real mismatch. Geo hints never
  // ride the attribute (deliveryUsAttrSync owns that rule).
  function deliveryUsAttrHeal() {
    try {
      var d = cfg && typeof cfg.delivery === 'object' ? cfg.delivery : null;
      var us = d ? d.us : null;
      if (!us || typeof us !== 'object' || us.enabled !== true) return;
      var choice = deliveryUsChoiceGet() || '';
      // RAW compare (review): ANY stored value that differs from the
      // choice — junk written by another app included — is rewritten
      // once; loop-free because the island reflects the written value on
      // the next render, and deliveryUsAttrSync validates what it posts.
      var attr = typeof d.usAttr === 'string' ? d.usAttr : '';
      if (choice !== attr) deliveryUsAttrSync(choice);
    } catch (e) { /* never break the theme */ }
  }
  ...
  // ============================================================ v14 rewards
  //
  // Set savings (KIT tiers) + gift tiers — SPEC-v14-rewards §4/§6/§7, v15
  // island isolation. The rewards config NEVER rides the shared
  // #cx-cart-config island any more: cart-booster.liquid emits a SEPARATE
  // <script type="application/json" id="cx-rw-config"> ONLY when a rewards
  // feature is LIVE-effective for the market (never on draft flags), so a
  // Liquid hiccup inside it can no longer blank the older cart widgets and
  // preview data never reaches live pages. Shape (both sources):
  ...
  // entries {t, h, variants:[{id, price, available, position}], s?}),
  // cached in sessionStorage for 10 minutes; until it lands the meter
  // renders without the gift's name/value and NO gift line is added or
  // removed. Per-market gift amounts / product exclusions are looked up by
  // market handle here (rwMarket(): the verified preview's simulated
  // market, else cfg.market) — a data lookup, never scope logic:
  // visibility still comes exclusively from cfg.effective / PREVIEW
  // through featureOn(). Every path fails closed: no config, no tiers, no
  // strings -> render nothing, mutate nothing. Cart mutations (KIT code
  // sync, gift add/remove) ride the quiet path, single-flight, one
  // correction per cart signature, and never inside a preview session
  // unless it is a live rehearsal ("Test with my real cart").
  ...
  // ---- v14 rewards: cart
  function readRwConfig() {
    // v15: the separate live-only rewards island (null when absent or
    // unparseable — a broken rewards island never touches the main one)
    var node = document.getElementById('cx-rw-config');
    if (!node) return null;
    try {
  ...
  var RW = readRwConfig(); // boot() swaps in PREVIEW.rw inside a verified preview
  ...
  // Lazily fetched gift-product data (v15): map "<numeric variantId>" ->
  // {h handle, t title, c cents, a available, p product position} for
  // EVERY variant of every gift-option / samplePool product, plus
  // first[handle] = the FIRST VARIANT BY POSITION (v15.1, availability
  // ignored; empty-variantId options resolve to it — the server's hv
  // resolves the same way when it builds the rewards metafield, so the
  // Function agrees; an unavailable one is refused by rwVariantOk). state:
  // '' (nothing to load) | 'loading' | 'ready' | 'failed'.
  var rwGiftData = { state: '', key: '', map: {}, first: {}, retryAt: 0 };
  function rwGifts() { return rwGiftData.map; }
  ...
  function rwSsTiers() {
    // KIT tiers from the active config (inside a verified preview RW is
    // the preview-config "rw" field with the draft tiers merged in);
    // every tier must be usable.
    var ss = rwSs();
    var tiers = ss && Array.isArray(ss.tiers) ? ss.tiers : [];
    return tiers.filter(function (tr) {
      return !!(tr && Number(tr.count) > 0 && Number(tr.pct) > 0 && typeof tr.code === 'string' && tr.code);
    });
  }

  function rwBlockedSet() {
    // v15.1 rewards.setSavings.blockedCodes (SERVER-written by Connect):
    // ladder codes whose Shopify code is owned by a FOREIGN discount. The
    // app never attaches one — that tier is simply unavailable (the best
    // lower unblocked tier applies, or none). Upper-cased set; the tier
    // rows themselves stay in rwSsTiers() (display only).
    var ss = rwSs();
    var list = ss && Array.isArray(ss.blockedCodes) ? ss.blockedCodes : [];
    var out = {};
  ...
  function rwUsableTiers() {
    // ladder tiers the app may attach: rwSsTiers() minus blocked codes
    var blocked = rwBlockedSet();
    return rwSsTiers().filter(function (tr) { return !blocked[String(tr.code).toUpperCase()]; });
  }

  function rwYieldCodes() {
    // v15 "codes we step aside for" (settings rewards.setSavings.
    // yieldToCodes, e.g. the merchant's own pre-existing KIT2..KIT10
    // discounts): when the shopper's cart carries one of them the app
    // never attaches its own ladder code and removes an already-attached
    // one — the app never touches the foreign discount itself. Upper-
    // cased, deduped, ladder codes dropped (the sanitizer does the same).
    var ss = rwSs();
    var list = ss && Array.isArray(ss.yieldToCodes) ? ss.yieldToCodes : [];
    var kit = {};
  ...
  function rwYieldOnCart() {
    // the first yield code present on the cart (applicable or not — the
    // shopper chose it; Shopify decides what it does), or ''. v15.1: a
    // BLOCKED ladder code on the cart is that same kind of foreign
    // discount (the merchant's colliding one) — the app steps aside for
    // it too rather than stacking its own lower tier on top.
    var codes = rwCartCodes();
    var yields = rwYieldCodes();
    var blocked = rwBlockedSet();
    for (var i = 0; i < codes.length; i++) {
      var up = codes[i].code.toUpperCase();
      if (blocked[up]) return codes[i].code;
      for (var j = 0; j < yields.length; j++) {
        if (up === yields[j]) return codes[i].code;
      }
    }
    return '';
  }
  ...
  function rwPool() {
    // samplePool -> [{vid numeric, h handle}] (unusable entries dropped; a
  ...
  function rwGiftThresholdCents(i) {
    ...
    // own amount when its currency IS the presentment currency (inside a
    // preview the config already carries the draft amounts for the
    // simulated market), else the EUR default × the guarded shop rate —
    // the thresholdCents() rule.
    var gt = rwGt();
    if (!gt) return 0;
    var tiers = rwGtTiers();
  ...
  function rwOptionVid(op) {
    // numeric variant id of a "variant" option: its variantId when set,
    // else (v15.1) the fetched FIRST variant by position of its handle — the
    // server resolves empty variantIds the same way at sync time, so the
    // Function and the storefront agree; '' while unknown
    if (!op || typeof op !== 'object') return '';
    var vid = rwNum(op.variantId);
    if (vid) return vid;
    return op.handle ? rwGiftData.first[String(op.handle)] || '' : '';
  }
  ...
  // ---- v15 lazy gift-product data (app proxy cart-data?handles=…)
  function rwGiftHandles() {
    // every gift-option handle (kind variant) + samplePool handle, order of
    // appearance, deduped, capped at the proxy's 20
    var out = [];
    var seen = {};
    var push = function (h) {
      if (typeof h !== 'string' || !h || seen[h] || out.length >= 20) return;
      seen[h] = true;
      out.push(h);
    };
    var gt = rwGt();
    ...
    for (var ti = 0; ti < tiers.length; ti++) {
      ...
      for (var si = 0; si < slots.length; si++) {
        ...
        for (var oi = 0; oi < ops.length; oi++) {
          if (ops[oi] && ops[oi].kind !== 'samples') push(ops[oi].handle);
        }
      }
    }
    var pool = gt && Array.isArray(gt.samplePool) ? gt.samplePool : [];
  ...
  function rwGiftIngest(by, handles) {
    // productsByHandle -> the vid map + first-variant index (only the
    // requested handles; unknown / null entries are simply absent)
    var map = {};
    var first = {};
    for (var i = 0; i < handles.length; i++) {
      var h = handles[i];
      var entry = by && by[h];
      if (!entry || typeof entry !== 'object' || !Array.isArray(entry.variants)) continue;
      var title = typeof entry.t === 'string' ? entry.t : '';
      for (var v = 0; v < entry.variants.length; v++) {
        var vr = entry.variants[v];
        var vid = vr ? rwNum(vr.id) : '';
        if (!vid) continue;
        var avail = vr.available !== false;
        map[vid] = { h: h, t: title, c: Math.round(Number(vr.price) || 0), a: avail };
        // v15.1: FIRST VARIANT BY POSITION regardless of availability —
        // the server's hv (rewards metafield / Function) resolves the
        // same way, so both sides always name the same variant; a sold-
        // out first variant is refused by rwVariantOk and the slot falls
        // to its next option, exactly as the Function would not grant it.
        if (!first[h]) first[h] = vid;
      }
    }
    return { map: map, first: first };
  }

  function rwGiftsReady() {
    // may the gift sync trust rwGifts()? — loaded, or there is really
    // nothing to load (no gift-option / samplePool handle at all). A pass
    // that runs before rwGiftLoad() ever did (state '' with handles to
    // fetch — v15.1 F1: pages without a drawer / cart table never reached
    // the render path that loads) must NOT read the empty map as "remove
    // everything".
    if (rwGiftData.state === 'ready') return true;
    return rwGiftData.state === '' && !rwGiftHandles().length;
  }

  function rwGiftLoad() {
    // Fill rwGiftData once per handle set: sessionStorage cache
    // ("cx_rw_gifts:<currency>:<locale>:<hash>", 10 minutes) first, else
    // ONE proxy request; a failed request stands down for 15 s, then
    // retries on a later pass. Re-renders (and lets the gift sync
    // re-plan) once data lands.
    if (!RW || !rwGt()) return;
    var handles = rwGiftHandles();
    if (!handles.length) { rwGiftData.state = ''; return; }
    // key = currency + storefront language + handle set (prices are
    // presentment, titles are localized — a switch must not serve stale data)
    var key = 'cx_rw_gifts:' + activeCurrency() + ':' + (typeof cfg.pageLocale === 'string' ? cfg.pageLocale : '') + ':' + cardFlagHash(handles.join(','));
    if (rwGiftData.key === key && (rwGiftData.state === 'ready' || rwGiftData.state === 'loading')) return;
    if (rwGiftData.key === key && rwGiftData.state === 'failed' && Date.now() < rwGiftData.retryAt) return;
    if (rwGiftData.key !== key) { rwGiftData.map = {}; rwGiftData.first = {}; } // new handle set: never serve another set's data
    rwGiftData.key = key;
    var cached = null;
    try {
      var raw = window.sessionStorage ? window.sessionStorage.getItem(key) : null;
    ...
    if (cached && typeof cached === 'object' && cached.map && typeof cached.map === 'object' && Number(cached.t) > Date.now() - 600000) {
      rwGiftData.map = cached.map;
      rwGiftData.first = cached.first && typeof cached.first === 'object' ? cached.first : {};
      rwGiftData.state = 'ready';
      return;
    }
    if (!window.fetch) { rwGiftData.state = 'failed'; rwGiftData.retryAt = Date.now() + 15000; return; }
    rwGiftData.state = 'loading';
    fetchHandleData(handles)
      .then(function (by) {
        if (rwGiftData.key !== key) return; // superseded by a config change
        var got = rwGiftIngest(by, handles);
        rwGiftData.map = got.map;
        rwGiftData.first = got.first;
        rwGiftData.state = 'ready';
        try { window.sessionStorage.setItem(key, JSON.stringify({ t: Date.now(), map: got.map, first: got.first })); } catch (e) { /* noop */ }
        rwGiftReplan();
        scheduleRefresh();
      })
      .catch(function () {
        if (rwGiftData.key !== key) return;
        rwGiftData.state = 'failed';
        rwGiftData.retryAt = Date.now() + 15000;
      });
  }
  ...
  function rwDesiredCode() {
    ...
    // the highest qualifying tier whose code is NOT blocked (v15.1: a
    // blocked tier is unavailable; the best lower unblocked one applies)
    var tier = cxRwTier(rwUsableTiers(), rwDistinctCount());
    return tier ? { code: String(tier.code), count: Number(tier.count), pct: Number(tier.pct) } : null;
  }

  function rwKitActive() {
    // the applied + applicable ladder code on the cart, or null ({code,
    // tier}); a yield code (rwYieldOnCart) is a foreign discount the app
    // knows nothing about — never "active" here; a BLOCKED ladder code on
    // the cart is a foreign discount too (v15.1) — ignored the same way
    var tiers = rwUsableTiers();
    var codes = rwCartCodes();
    for (var i = 0; i < codes.length; i++) {
      if (!codes[i].applicable) continue;
      for (var j = 0; j < tiers.length; j++) {
        if (String(tiers[j].code).toUpperCase() === codes[i].code.toUpperCase()) return { code: codes[i].code, tier: tiers[j] };
      }
    }
    return null;
  }
  ...
  function rwSyncCode() {
    ...
    // v15 yieldToCodes (rwYieldCodes): a yield code on the cart (the
    // merchant's own pre-existing KIT2..KIT10 discount, applicable or not)
    // -> desired = null: no ladder code is added and an already-attached
    // ladder code is removed once; the yield code and every other foreign
    // code are kept untouched. The app never attaches a yield code.
    // v15.1 blockedCodes: a blocked ladder code is a FOREIGN discount —
    // never attached, never removed (kept like any shopper code); the
    // desired tier is the best unblocked one (rwDesiredCode).
    try {
      ...
      var has = false;
      var stale = false;
      var yielding = !!rwYieldOnCart();
      var want = yielding ? null : rwDesiredCode();
      var sig = cartSignature() + '|' + (yielding ? '*yield' : (want ? want.code : ''));
      ...
      cartRequest('cart/update.js', { discount: keep.join(',') })
        ...
        .then(function (cart) {
          ...
          if (want) {
            ...
            if (applied && applied.code.toUpperCase() === want.code.toUpperCase()) {
  ...
  function rwSyncGifts() {
    ...
    try {
      ...
      rwNoteKnownGifts();
      // v15: gift product data arrives lazily (rwGiftLoad) — until it is
      // known no gift line is added OR removed (an unknown wanted set must
      // never read as "remove everything"); the pass re-runs once it lands.
      // v15.1 (F1): load HERE too, not only from the drawer render — a page
      // without a drawer / cart table still runs the sync and must fetch
      // (cache / one proxy request, idempotent) before it may plan.
      rwGiftLoad();
      if (!rwGiftsReady()) return;
      var plan = rwGiftPlan();
  ...
  function rwRenderMeter(container, suppressShip) {
    ...
    try {
      ...
      if (!next) {
        ...
        var label = rwTierLabel(head.i);
        // title still unknown (v15 lazy product data): the generic tag,
        // lower-cased inside the sentence, until the fetch re-renders
        if (label === rwT('gift_tag')) label = label.toLowerCase();
        var params = { gift: label, value: money(value) };
      ...
      if (PREVIEW && plan.add.length && !rwMutable()) {
        ...
        wrap.appendChild(el('div', 'cx-preview-note', 'Preview sample — real visitors get this gift added to their cart automatically (tick "Test with my real cart" in the Preview Center to add it for real).'));
      }
      container.appendChild(wrap);
      var keys = ['gift_tiers'];
  ...
  function rwSnapshot() {
    ...
    state.rw = null;
    if (!RW) return;
    if (featureOn('giftTiers')) rwGiftLoad(); // v15: lazy gift-product data (cache / one proxy request)
    var skipV = {};
    var skipH = {};
    var skipK = {};
      ...
      .then(function (out) {
        if (out.status === 200 && out.body && out.body.valid === true) {
          ...
          PREVIEW = {
            ...
            // rehearsal ("Test with my real cart": real cart mutations
            // allowed).
            sim: rwSimRead(data.simCart),
            rehearsal: data.rehearsal === true,
            // v15: the simulated market's rewards sections ({ss, gt, paused,
            // market}) with the draft tiers / amounts merged in — replaces
            // the live-only #cx-rw-config island for this verified session
            rw: data.rw && typeof data.rw === 'object' && (data.rw.ss || data.rw.gt) ? data.rw : null
          };
          // v15.1 (F3): the server always ships an rw block; swap it in
          // ONLY when a rewards feature is live-in-market or drafted for
          // this session — otherwise the live island (or null) stays and
          // the preview does not switch subtotal math / skip sets / purge
          // paths on for a merchant who never asked for rewards.
          if (PREVIEW.rw && (featureOn('setSavings') || featureOn('giftTiers'))) RW = PREVIEW.rw;
          else PREVIEW.rw = null;
  ...
  function init() {
    ...
    initCardFlags();
    // v13: keep the cart's _cx_us_state attribute coherent with the
    // stored choice — BEFORE the live-widget bail below (checkout
    // coherence matters even on pages with zero live cart widgets).
    // Prime first: a stale choice of a now-hidden state must self-clear
    // before the mismatch verdict, so one write fires instead of two.
    deliveryUsPrime();
    deliveryUsAttrHeal();
