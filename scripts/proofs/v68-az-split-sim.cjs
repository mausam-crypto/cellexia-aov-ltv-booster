 ...
 * extraction, the house sim convention; the v6.1 v61-az-module sim this
 * replaces lived in a since-wiped session scratchpad — which is why this
 * one is committed to the repo; run via `npm run proofs`).
 *
...
// Static evidence pins (the coverage-tripwire style, re-homed here since
// the harness is gone): the ships line root must carry its own feature
// attribute, and the file must gate each line on its own key.
const PINS = [
