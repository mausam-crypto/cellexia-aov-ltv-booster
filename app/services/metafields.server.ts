...
export interface RewardsMetafieldContext {
   ...
   *  — RewardsState.nodes.hv. Resolves gift options that carry a handle but
   *  no variantId (the shipped presets) so the Function knows the variant the
   *  storefront will add (it picks the first available variant from
   *  cart-data — same rule). Unresolvable options are omitted from the
   *  metafield (the storefront may only add metafield-known variants). */
...
function projectRewards(
  ...
  // handle resolved through hv (first available variant at the last unit-map
  // refresh), else "" (dropped from the projection).
  const optionVid = (option: { variantId: string; handle: string }): string =>
...
async function loadRewardsMaps(
  shop: string,
): Promise<Pick<RewardsMetafieldContext, "units" | "vp" | "hv">> {
  try {
    ...
    if (nodes.hv && typeof nodes.hv === "object") {
      for (const [handle, vid] of Object.entries(nodes.hv as Record<string, unknown>)) {
        if (typeof vid === "string" && /^\d+$/.test(vid)) hv[handle] = vid;
      }
    }
    return { units, vp, hv };
  } catch {
...
/**
 * Result of a settings sync. `errors` = the two CONFIG MIRRORS (app-data
 * `cellexia/config` + shop `$app:cellexia/config`) failed — the storefront /
 * checkout would run on stale settings. `warnings` = the mirrors were
 * written but the SEPARATE `$app:cellexia/rewards` write (the Discount
 * Function's config) failed — the caller must surface it (Preview Center
 * banner, Rewards page) but the mirrors are already in place: a disarm has
 * ALREADY written armed:false to both mirrors even when this warning is
 * present (v15 incident guarantee).
 */
...
export async function syncSettingsToMetafields(
  ...
  if (!appInstallationId || !shopId) {
    return { ok: false, errors: ["Could not resolve metafield owner ids"], warnings: [] };
  }
  ...
  // STEP 1 (v15): the two config mirrors in ONE metafieldsSet call. This is
  // the write that arms/disarms the storefront + checkout — it must never
  // wait on, or fail because of, the rewards projection below.
  const mirrors = await admin.graphql(METAFIELDS_SET_MUTATION, {
    variables: {
      metafields: [
        {
          ownerId: appInstallationId,
  ...
  const mirrorsJson = (await mirrors.json()) as {
    data?: {
      metafieldsSet?: {
        userErrors?: { field?: string[] | null; message: string }[];
      };
  ...
  const errors = [
    ...(mirrorsJson.errors ?? []).map((e) => e.message),
    ...(mirrorsJson.data?.metafieldsSet?.userErrors ?? []).map((e) => e.message),
  ];
  if (errors.length > 0) {
    // The mirrors did not land: report loudly; the rewards write is skipped
    // (a Function config newer than the settings it mirrors is worse than a
    // stale one).
    return { ok: false, errors, warnings: [] };
  }

  // STEP 2 (v15): the Discount Function's rewards projection (SPEC §2.2) in a
  // SEPARATE metafieldsSet call. Unit/variant/handle maps come from
  // RewardsState (no API call); the country→market map from marketCountryMap
  // (per-shop 1 h cache). ANY failure here — a thrown transport error, a
  // GraphQL error, a userError — is returned as a WARNING string and never
  // undoes or blocks step 1.
  const warnings: string[] = [];
  try {
    let cm: Record<string, string> = {};
    let maps: Pick<RewardsMetafieldContext, "units" | "vp" | "hv"> = {};
    if (shopDomain) {
    ...
    for (const message of rewardsErrors) {
      warnings.push(`Rewards discount config (metafield $app:cellexia/rewards) not written: ${message}`);
    }
  } catch (error) {
    warnings.push(
      `Rewards discount config (metafield $app:cellexia/rewards) not written: ${
