...
// ===================================================== 1. Liquid byte budget
{
  ...
  // gift map, 16 rw.* strings) + the amazon-booster set-savings island
...
// structural tripwires so a refactor cannot silently drop a wire.
{
  ...
  const RW_GATE = "{%- if cx_eff_ss or cx_eff_gt or cx_draft_rw %}";
  const RW_MEMBER = '"rw": {"ss": {{ cx_rw.setSavings | json }}, "gt": {{ cx_gt | json }}, "paused": {{ app.metafields.cellexia.gift_stock.value.paused[cx_market] | json }}, "gifts": {"_": 0';
  const rwGateAt = cartLiquid14.indexOf(RW_GATE);
  const rwMemberAt = cartLiquid14.indexOf(RW_MEMBER);
  ok(rwGateAt !== -1, "v14: cart island rw gate present (effective OR draft-armed)");
  ok(rwMemberAt !== -1, "v14: cart island emits the \"rw\" member (ss/gt verbatim, paused, gifts map)");
  ok(
    rwGateAt !== -1 && rwMemberAt !== -1 && rwGateAt < rwMemberAt && cartLiquid14.indexOf("{%- endif %}", rwGateAt) > rwMemberAt,
    "v14: the \"rw\" member sits INSIDE its gate (no rw leak to ungated visitors)",
  );
  ok(
    (cartLiquid14.match(new RegExp(RW_GATE.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g")) ?? []).length === 2,
    "v14: exactly two rw gates in cart-booster.liquid (island member + rw.* strings loop)",
  );
  ok(
  ...
  const AZ_RW = '{%- if az_any_ss %}\n"rw": {"live": {{ az_eff_ss }}, "tiers": {{ cx_rw.tiers | json }}, "sf": {"pdp": {{ cx_rw.surfaces.pdpLine | json }}, "sim": {{ cx_rw.surfaces.similarCaption | json }}, "fbt": {{ cx_rw.surfaces.fbtCaption | json }}}, "excl": {{ cx_rw.setSavingsExcludedByMarket[cx_market] | json }}, "giftPids": [';
  ok(azLiquid14.includes(AZ_RW), "v14: amazon island \"rw\" member (live/tiers/sf/excl/giftPids) gated on az_any_ss");
  ...
  ok(
    cartJs14.includes("var RW = cfg.rw && typeof cfg.rw === 'object' ? cfg.rw : null;"),
    "v14: cellexia-cart.js reads cfg.rw once behind an object guard",
  );
  ...
  // v14.2 presets + legacy sweep / v14.3 alias codes: the settings model
  // exports both preset tables + aliasCodesFor; Connect creates/updates the
  // alias codes through the SAME job loop as the ladder codes and only
  // DEACTIVATES (discountCodeDeactivate, never delete) the legacy basic KIT
  // codes when "Keep legacy codes" is OFF and replaceExisting is ticked.
  const settingsSrc142 = read("app/models/settings.server.ts");
  for (const sym of ["LADDER_PRESETS", "LADDER_PRESET_KEYS", "GIFT_PRESETS", "GIFT_PRESET_KEYS", "LEGACY_KIT_CODES"]) {
    ok(settingsSrc142.includes(`export const ${sym} = `), `v14.2: settings.server.ts exports ${sym}`);
  }
  ok(settingsSrc142.includes("export function aliasCodesFor("), "v14.3: settings.server.ts exports aliasCodesFor(settings)");
  ok(settingsSrc142.includes("keepLegacyCodes: true,") && settingsSrc142.includes("ss.aliasCodes = aliasCodesFor(next);"), "v14.3: keepLegacyCodes defaults true and sanitizeSettings recomputes the derived aliasCodes");
  ok(rewardsSrv.includes("discountCodeDeactivate(id: $id)"), "v14.2: rewards.server.ts carries the discountCodeDeactivate mutation (Admin 2025-10)");
  const sweepAt = rewardsSrv.indexOf("if (!ss.keepLegacyCodes && options.replaceExisting) {\n    for (const code of LEGACY_KIT_CODES) {");
  ok(sweepAt > 0, "v14.3: legacy KIT sweep runs only when keepLegacyCodes is OFF and replaceExisting is ticked");
  ok(rewardsSrv.includes("notes.push(`deactivated legacy code ${code}.`);"), "v14.2: legacy sweep reports 'deactivated legacy code X' in the summary");
  ok(!rewardsSrv.slice(sweepAt, sweepAt + 2500).includes("CODE_DELETE"), "v14.2: legacy sweep never deletes (deactivate only)");
  ok(rewardsSrv.includes("const aliasCodes = aliasCodesFor(settings);") && rewardsSrv.includes("title: `Set savings alias ${code}`,"), "v14.3: Connect builds alias code jobs (title 'Set savings alias KITn') from aliasCodesFor");
  ok(rewardsSrv.includes("`created ${label}.`") && rewardsSrv.includes("`updated ${label}.`") && rewardsSrv.includes("const label = job.alias ? `alias code ${job.code}` : job.code;"), "v14.3: Connect summary says 'created alias code KITn.' / 'updated alias code KITn.'");
  ok(read("app/services/health.server.ts").includes("for (const code of aliasCodesFor(settings)) {"), "v14.3: rewards-discounts health check requires the alias nodes while keepLegacyCodes");
  const mfAlias = read("app/services/metafields.server.ts");
  ok(mfAlias.includes("alias: aliasCodesFor({") && mfAlias.includes("alias: string[];"), "v14.3: rewards metafield ss carries `alias` (live + draft via projectRewards)");
  const logicAlias = read("extensions/cellexia-rewards/src/logic.js");
  ok(logicAlias.includes("codeIn(ss.alias, code)"), "v14.3: logic.js computeKit references cfg.ss.alias (alias grants the qualifying tier)");
  // Metafields: the third metafieldsSet entry + the app-data gift_stock.
  const mfSrc14 = read("app/services/metafields.server.ts");
  ok(mfSrc14.includes('namespace: "$app:cellexia",\n          key: "rewards",'), "v14: syncSettingsToMetafields writes $app:cellexia/rewards");
  ...
  for (const k of ["simCart:", "rehearsal:", "rewardsForMarket:"]) {
    ok(pc14.includes(k), `v14: proxy.preview-config emits ${k.slice(0, -1)}`);
  }
