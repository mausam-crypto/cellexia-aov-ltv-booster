...
- **harness.mjs** — structural tripwires, each self-checking against
  ...
  exemptions + RW_DEFAULTS parity, gen-ships LOCALE_BYTE_BUDGET).
...
- **sims/** — engine/feature suites that vm-extract the REAL shipped
  ...
  - **rewards-function.mjs** — v14: the REAL Discount Function pure logic
    ...
    directly): config/draft selection, KIT tier grant/refuse, gift grants
    (cumulative on/off, samples/max-lines caps, market amounts vs EUR
    fallback), free-shipping units/threshold + cheapest option, wrong
    class → nothing, wrappers never throw.
