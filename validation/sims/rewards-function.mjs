 ...
 *    is the fallback; KIT codes compare case-insensitively; v14.3 alias
 *    codes (ss.alias) grant the qualifying tier, ladder codes stay
 *    exact-match; a per-market
...
{
  // v14.3 alias codes: LIVE ladder is extended (KIT2/3/5/10) → KIT4/KIT6 are aliases.
  const aliased = clone(LIVE);
  aliased.ss.alias = ["KIT4", "KIT6"];
  const three = [line(11, 1, 57), line(21, 2, 57), line(31, 3, 57)];
  const two = [line(11, 1, 57), line(21, 2, 57)];
  const a3 = kitCand(computeKit(aliased, input({ lines: three, code: "KIT4" })));
  ok(a3 && a3.value.percentage.value === "10.0" && a3.associatedDiscountCode.code === "KIT4", "v14.3 alias KIT4 grants the qualifying tier (KIT3 = 10.0) on 3 products");
  ok(kitCand(computeKit(aliased, input({ lines: two, code: "KIT6" })))?.value.percentage.value === "5.0", "v14.3 alias KIT6 grants the qualifying tier (KIT2 = 5.0) on 2 products");
  ok(kitCand(computeKit(aliased, input({ lines: two, code: "kit6" })))?.value.percentage.value === "5.0", "v14.3 alias compared case-insensitively (kit6)");
  ok(computeKit(aliased, input({ lines: [line(11, 1, 57)], code: "KIT4" })).length === 0, "v14.3 alias with no qualifying tier (1 product) grants nothing");
  ok(computeKit(aliased, input({ lines: three, code: "KIT2" })).length === 0, "v14.3 ladder code KIT2 still exact-match only (cart qualifies for KIT3)");
  ok(computeKit(aliased, input({ lines: three, code: "KIT5" })).length === 0, "v14.3 ladder code KIT5 refused on 3 products (ladder codes never behave as aliases)");
  ok(computeKit(aliased, input({ lines: three, code: "KIT7" })).length === 0, "v14.3 unknown code (neither ladder nor alias) grants nothing");
  ok(computeKit(LIVE, input({ lines: three, code: "KIT4" })).length === 0, "v14.3 no alias list (pre-v14.3 metafield) → non-ladder code grants nothing");
  const emptyAlias = clone(aliased);
  emptyAlias.ss.alias = [];
  ok(computeKit(emptyAlias, input({ lines: three, code: "KIT4" })).length === 0, "v14.3 keepLegacyCodes off (alias []) → KIT4 grants nothing");
}
{
  // subscription toggle
