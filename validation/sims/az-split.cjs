...
// v8.16b: the ships-from grammar tables ride the JS asset (generated
// single-line literal) — extracted too, so the G-cases run against the
// REAL shipped forms.
const EXTRACTED = [
...
// --- preview draft convention ---------------------------------------------------
{
  ...
  // v8.16: the en grammar table upgrades NL to its article form — the
  // line reads "Ships from the Netherlands", not the bare CLDR name.
  ok(node.querySelector(".cx-az-stock__ships").textContent === "Ships from the Netherlands", "P1 warehouse from the ships member (the Netherlands — en table form)");
...
// v8.16b contract: the inflected country phrase comes from the REAL
// AZ_SHIPS_FORMS tables shipped inside cellexia-pdp.js (generated by
// scripts/gen-ships-from-grammar.mjs; extracted above), keyed
// [pageLocale][warehouse]. A table hit rides the NATURAL sentence; a miss
// (warehouse outside the set, unsupported page language) uses the
// label-style ships_from_fallback with the bare Intl name; with neither
// string, the pre-v8.16 natural+bare behavior (F1/S2 pin that path).
{
  const G_STRINGS = {
    "amazon.ships_from": "Versand aus @@COUNTRY@@",
    "amazon.ships_from_fallback": "Versand aus: @@COUNTRY@@",
  };
  const DE_CO = new Intl.DisplayNames(["de"], { type: "region" }).of("CO"); // Kolumbien

  // G1 subtle + table hit: German page, Swiss warehouse — the REAL de.CH
  // entry (dative article) renders in the natural sentence.
  {
    const cfg = cfgFor(false, true, "CH");
    cfg.pageLocale = "de";
    ...
    ok(!!ships && ships.textContent === "Versand aus der Schweiz",
      "G1 form path: real de.CH table form in the natural sentence (got " + (ships && ships.textContent) + ")");
  }

  // G2 subtle + warehouse OUTSIDE the table: label line with the bare
  // Intl name in the page language.
  {
    const cfg = cfgFor(false, true, "CO");
    cfg.pageLocale = "de";
    Object.assign(cfg.strings, G_STRINGS);
    const { page } = runScenario(cfg);
    const ships = page.document.querySelector(".cx-az-stock__ships");
    ok(!!ships && ships.textContent === "Versand aus: " + DE_CO,
      "G2 fallback path: table-miss warehouse -> label template with the bare Intl name");
  }

  // G2b an UNSUPPORTED page language (no table) falls back the same way.
  {
    const cfg = cfgFor(false, true, "CH");
    cfg.pageLocale = "zh";
    Object.assign(cfg.strings, G_STRINGS);
    const zhCH = new Intl.DisplayNames(["zh"], { type: "region" }).of("CH");
    const { page } = runScenario(cfg);
    const ships = page.document.querySelector(".cx-az-stock__ships");
    ok(!!ships && ships.textContent === "Versand aus: " + zhCH,
      "G2b unsupported page language: fallback label with that language's Intl name");
  }

  // G3 table miss AND no fallback string: the pre-v8.16 natural+bare line.
  {
    const cfg = cfgFor(false, true, "CO");
    cfg.pageLocale = "de";
  ...
  // G4 prominent + table hit: the WHOLE inflected phrase bolds (article
  ...
  // G5 prominent + table miss: the fallback label template is the one
  // that splits — the bare name bolds inside the label line.
  {
    const cfg = cfgFor(false, true, "CO");
    cfg.pageLocale = "de";
    cfg.ships.f = "p";
    ...
    const text = page.document.querySelector(".cx-az-stock__ships-text");
    const country = text && text.querySelector(".cx-az-stock__ships-country");
    ok(!!country && country.textContent === DE_CO,
      "G5 prominent fallback: bare name bolds");
    ok(!!text && text.textContent === "Versand aus: " + DE_CO,
      "G5 prominent fallback: label sentence recomposed");
  }

  // G6 regional page-locale key: pt-PT resolves its own table (fused
  // preposition entry) — the exact-key branch of the locale lookup.
  {
    const cfg = cfgFor(false, true, "CH");
    cfg.pageLocale = "pt-PT";
    cfg.strings["amazon.ships_from"] = "Enviado @@COUNTRY@@";
    cfg.strings["amazon.ships_from_fallback"] = "Enviado de: @@COUNTRY@@";
    const { page } = runScenario(cfg);
    const ships = page.document.querySelector(".cx-az-stock__ships");
    ok(!!ships && ships.textContent === "Enviado da Suíça",
      "G6 regional locale key: pt-PT table hit (fused preposition)");
  }
}
...
// no label resolves (the CSS [hidden] guard covers the display:flex trap).
{
  ...
  ok(src.includes("? azShipsCompose(sfWh, label)\n          : azT('amazon.ships_from', { country: label });"),
