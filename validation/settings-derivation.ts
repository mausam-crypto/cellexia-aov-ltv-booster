...
ok(FEATURE_KEYS.length === 47, `FEATURE_KEYS has 47 keys (got ${FEATURE_KEYS.length})`);
...
ok(new Set(FEATURE_KEYS).size === 47, "FEATURE_KEYS has no duplicates");
...
{
  ok(
    FEATURE_KEYS[FEATURE_KEYS.length - 12] === "set_savings" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 11] === "gift_tiers" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 10] === "buy_box_proof" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 9] === "image_badges" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 8] === "cart_overlay_fix" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 7] === "cart_compact" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 6] === "cart_pinned_checkout" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 5] === "quantity_selector" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 4] === "research_band" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 3] === "award_strip" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 2] === "atc_button" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 1] === "quantity_sync",
    "v14/v19/v20/v21/v26/v29/v31: set_savings + gift_tiers, then buy_box_proof, then image_badges, then the three v21 overlay keys, then quantity_selector, then the two v29 split keys, then the v31 pair, are the LAST FeatureKeys (appended, never inserted)",
  );
