...
ok(FEATURE_KEYS.length === 45, `FEATURE_KEYS has 45 keys (got ${FEATURE_KEYS.length})`);
...
ok(new Set(FEATURE_KEYS).size === 45, "FEATURE_KEYS has no duplicates");
...
{
  ok(
    FEATURE_KEYS[FEATURE_KEYS.length - 10] === "set_savings" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 9] === "gift_tiers" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 8] === "buy_box_proof" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 7] === "image_badges" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 6] === "cart_overlay_fix" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 5] === "cart_compact" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 4] === "cart_pinned_checkout" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 3] === "quantity_selector" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 2] === "research_band" &&
      FEATURE_KEYS[FEATURE_KEYS.length - 1] === "award_strip",
    "v14/v19/v20/v21/v26/v29: set_savings + gift_tiers, then buy_box_proof, then image_badges, then the three v21 overlay keys, then quantity_selector, then the two v29 split keys, are the LAST FeatureKeys (appended, never inserted)",
  );
