...
## v15 — preview isolation, app-owned codes, plain admin (2026-08-17)

Binding on top of everything above; where v15 contradicts an earlier
section, v15 wins.

### Incident that triggered v15

Arming a Preview with the new draft flags made the LIVE cart booster
disappear for every visitor ("cart became empty"), and disarming did not
restore it. Root cause class: v14 emitted a large `rw` data block (with
`all_products` look-ups and a resolved gifts map) INSIDE the shared
`#cx-cart-config` JSON island for EVERY visitor whenever a preview was armed
with the draft flags; a single Liquid problem inside that block invalidated
the whole island, so every older cart widget that reads the island vanished
too — and preview data reached live pages. Two more merchant findings: the
discount-code approach (replace/deactivate existing KIT codes, alias codes)
is unacceptable — NO existing discount may ever be touched — and the admin
page had become jargon.

### 1. Settings (settings.server.ts)

- REMOVED: `rewards.setSavings.keepLegacyCodes`, `aliasCodes`,
  `aliasCodesFor()`, `LEGACY_KIT_CODES`. The sanitizer silently ignores /
  deletes those keys in stored JSON (mergeSettings drops unknown keys; the
  rewards block also deletes them from a spread patch).
- `LADDER_PRESETS` codes are APP-OWNED NEW codes: compact = SET2/SET3/SET4/
  SET6 (5/10/15/20 %), extended = SET2/SET3/SET5/SET10 (5/10/20/30 %). No
  preset ever ships a KIT code. Existing rows keep their saved tiers.
- NEW `rewards.setSavings.yieldToCodes: string[]` — "codes we step aside
  for". Default `DEFAULT_YIELD_TO_CODES = ["KIT2","KIT3","KIT5","KIT10"]`
  (the store's historical codes). `sanitizeYieldToCodes(raw, ladder)`: ≤ 20
  entries (`REWARDS_CAPS.yieldToCodes`), trimmed, upper-cased,
  `/^[A-Z0-9_-]{2,32}$/`, deduped, and any LADDER TIER CODE is dropped; a
  non-array falls back to the defaults, `[]` is a real value.
  `validateSetSavingsPatch` fails loud on a non-list, a malformed entry,
  > 20 entries, or a yield code that is also a tier code of the same patch.
  Semantics: if the shopper's cart carries one of these codes, the app never
  attaches its own SET code and removes an already-attached SET code
  (storefront `rwSyncCode` + checkout RewardsSafetyNet). The app never
  creates, updates or deletes those discounts.
- FEATURE_DEFS label for `set_savings` is now "Set savings" (no "KIT").
- **v15.1** FEATURE_DEFS label for `gift_tiers` is "Free gifts" (the
  Preview Center picker words match every other surface).
- **v15.1** NEW `rewards.setSavings.blockedCodes: string[]` — SERVER-WRITTEN
  by `connectRewardsDiscounts` = the ladder tier codes whose Shopify code is
  owned by a foreign discount (the collisions reported with
  `foreignCodeMessage`), `[]` when none. `sanitizeBlockedCodes(raw, ladder)`:
  trimmed, upper-cased, deduped, ≤ 6 (`REWARDS_CAPS.blockedCodes`), ONLY
  codes present in the (sanitized) ladder; a non-array → `[]`. Default `[]`.
  `validateSetSavingsPatch` tolerates an echoed list of strings (the admin
  shows the field read-only) and rejects a non-list / non-string entries.
  Semantics: storefront (`rwSyncCode` / `rwDesiredCode`) and checkout
  (`RewardsSafetyNet`) NEVER attach a blocked code — that tier is
  unavailable; the best lower tier whose code is not blocked is used; none →
  no code. The field rides in the config mirrors and in `#cx-rw-config`
  (`ss` = the raw section).

### 2. Discounts (rewards.server.ts)

`connectRewardsDiscounts(admin, shop, settings)` — NO options. The routine
NEVER deletes, deactivates or updates a discount that is not ours. Ours = a
`DiscountCodeApp` / `DiscountAutomaticApp` whose id is recorded in
`RewardsState.nodes` OR whose `appDiscountType.functionId` equals our
deployed function (`isOurCodeNode(node, functionId, knownIds)`). Per ladder
code: `codeDiscountNodeByCode(code)` → foreign owner → push
`foreignCodeMessage(code)` = "Code SET2 is already used by another discount
in your store. Change the code in the table or delete that discount
yourself; the app never touches discounts it did not create." and skip;
ours → `discountCodeAppUpdate`; none → `discountCodeAppCreate` (title
`Set savings SET2`). Automatic nodes: update the recorded id (recreate when
deleted by hand), nothing else. The legacy sweep, alias jobs and the
`discountCodeDelete` / `discountCodeDeactivate` mutations are GONE from the
module (the harness pins the absence of their names). Codes removed from the
tier table are forgotten, never deleted.

NEW `detectStoreCodes(admin, prefixes, {functionId?})` → up to 25 existing
code strings starting with any prefix (upper-cased, sorted). Uses
`discountNodes(first: 50, query: "method:code")` (the search syntax has no
`code:` filter — verified against the docs) with the per-type
`codes(first: 5)` connections, up to 4 pages, client-side prefix match;
skips EXPIRED discounts and our own Function-backed codes. Read-only.

`RewardsNodes` / `UnitMap` gain `hv: Record<handle, numeric variantId>` =
**v15.1: the FIRST VARIANT BY POSITION of every active product, regardless
of availability** (the v15.0 first-available rule is gone; `availableForSale`
is no longer queried — the harness pins its absence). Written by
`buildUnitMap` (Connect + Rewards-page save + `refreshUnitMap`).
`giftVariantGids(settings, hv)` resolves handle-only gift options through it
so the stock watcher and the metafield agree. Rationale: deterministic and
time-independent on both sides — the storefront resolves a handle-only
option to the same first variant (`first[handle]` = first variant by
position from cart-data), still refuses to add an unavailable variant and
falls to the next OPTION, exactly as the Function (which only knows that
vid) would not grant it either; the stock watcher pauses the same vid on
both sides.

**v15.1** Connect persists `rewards.setSavings.blockedCodes` = the collided
codes (empty when none) through `saveSettings` AFTER the tier loop and
BEFORE the metafield sync (which mirrors the UPDATED settings). A failed
save is an `errors[]` entry.

Health check `rewards-discounts`: alias requirement removed; fix hints say
"Create discount codes" and repeat the foreign-code rule.

### 3. Rewards metafield (metafields.server.ts)

`ss.alias` removed; every other key unchanged (`giftPids` stays). Variant
options with an empty `variantId` resolve to `hv[handle]` (contract §5
parity — v15.1: the storefront picks the FIRST cart-data variant by
position, the server the first variant by position at the last unit-map
refresh; availability plays no part in WHICH variant); an unresolvable
option is DROPPED from `gt.tiers[].slots` (the Function
never sees `vid: ""`).

`syncSettingsToMetafields` returns `{ok, errors, warnings}` and writes in
TWO calls: (1) the two config mirrors (`cellexia/config` app-data +
`$app:cellexia/config` shop) in ONE `metafieldsSet`; (2) the
`$app:cellexia/rewards` metafield in a SECOND, separate `metafieldsSet`. Any
failure of (2) — thrown, GraphQL error, userError — is returned as a
`warnings[]` string and never blocks or undoes (1); a failure of (1) skips
(2) and is an `errors[]` entry. Consequence: disarm ALWAYS writes
`armed:false` to both mirrors even if the rewards write fails. The
`gift_stock` write is unchanged. `PreviewSyncResult` gains `warnings`;
Connect surfaces sync warnings as errors (the rewards metafield IS its
point).

### 4. Liquid isolation

`cart-booster.liquid`: the `"rw"` member is REMOVED from `#cx-cart-config`.
ONLY when `cx_eff_ss or cx_eff_gt` (LIVE effective for the market — never
the draft flags) the block emits, immediately after the main island,
`<script type="application/json" id="cx-rw-config">{"ss": {{
cx_rw.setSavings | json }}, "gt": {{ cx_rw.giftTiers | json }}, "paused": {{
app.metafields.cellexia.gift_stock.value.paused[cx_market] | json
}}}</script>` — no `all_products` loops, no gifts map, no captures. The main
island keeps `effective.setSavings/giftTiers` and the `rw.*` strings (gated
on `cx_eff_ss or cx_eff_gt or cx_draft_rw` — inert text). `cx_draft_rw`
appears exactly 3× (assign, `cx_cart_draft_any`, strings gate).
`amazon-booster.liquid`: the `"rw"` member is emitted ONLY when `az_eff_ss`
and carries `{"live": true, "tiers", "sf", "excl"}` (no `giftPids`, no
`all_products`); the `rw.*` PDP strings keep riding `az_any_ss`.

### 5. Storefront (cellexia-cart.js / cellexia-pdp.js)

`RW = JSON.parse(#cx-rw-config)` when present (`readRwConfig()`), else
`PREVIEW.rw` inside a verified preview. Gift product data (title, price,
availability, handle) is no longer in Liquid: `rwGifts()` is filled lazily
from `GET apps/cellexia/cart-data?handles=<gift-option + samplePool handles,
deduped, ≤ 20>` (cache `sessionStorage cx_rw_gifts:<hash>` 10 min); until
loaded the meter renders the plain "away from a free gift" sentence and
re-renders once data arrives. Options with an empty `variantId` resolve to
the fetched FIRST variant by position (v15.1; server twin: `hv`, §2/§3 — an
unavailable one is refused by `rwVariantOk` and the next option is tried).
**v15.1** `rwDesiredCode` skips tiers whose code is in `ss.blockedCodes`
(best lower unblocked tier, else no code). yieldToCodes
rule (`rwYieldCodes()` / `rwSyncCode`): a yield code on the cart → want =
null and our ladder code is removed if present; never attach. All alias
logic removed. PDP: rw config = island `rw` when live, else derived from
`PREVIEW.rw.ss` (tiers, `surfaces` → `sf {pdp,sim,fbt}`,
`setSavingsExcludedByMarket[market]` → `excl`); `giftPids` usage removed.

### 6. Preview (preview.server.ts / proxy.preview-config.tsx / app.preview.tsx)

preview-config JSON gains `rw = rewardsPreviewSections(settings, draftConfig,
market, pausedByMarket)` = `{ss, gt, paused, market}`: the RAW
`rewards.setSavings` / `rewards.giftTiers` sections with the armed draft
tiers / amounts merged in (`setSavingsTiers` → `ss.tiers`, `giftTiers` →
`gt.tiers`, `giftAmountsByMarket` merged over `gt.giftThresholdsByMarket`),
`paused` = the market's paused gift variants from RewardsState (or `[]`).
`rewardsForMarket` stays for backward compat. This token-verified endpoint
is the ONLY way draft rewards data reaches a browser. `draftConfig.rehearsal`
keeps its key; the UI calls it "Test with my real cart" (explicit warning,
default unchecked). Arm/disarm/apply MUST surface `sync.warnings` in the
Preview Center banner.

**v15.1** `disarmPreview` retries `resyncMetafields` ONCE when the first
mirror write returns `!ok` (the DB row is disarmed before the write, so a
failed write leaves the storefront armed). Arm / disarm / rotate / apply
results carry `syncOk` (= `sync.ok`) and the Preview Center never prints
"shoppers are safe" when `syncOk` is false ("Shoppers may still see the
preview — press Disarm again now"). In a verified preview session the
storefront replaces `RW` by `PREVIEW.rw` ONLY when
`featureOn('setSavings') || featureOn('giftTiers')` (live ∪ draft flags).
Readiness copy never shows a health-check id ("see Setup & health →
Discount codes"; the `rewards-discounts` row is labelled "Discount codes"
and names blocked codes).

### 7. Checkout (RewardsSafetyNet)

Alias yield replaced by yieldToCodes: a yield code present together with our
ladder code → remove OUR ladder code once; the SET attach still happens only
when the checkout has NO code at all; gift honesty unchanged.

### 8. Health (health.server.ts)

NEW check `storefront-islands` (17 checks now): reuses ONE storefront fetch
per run (`loadStorefrontPages` → home + first product page, shared with
`deployed-extension`); `inspectStorefrontIslands(html)` JSON.parses every
`#cx-cart-config` / `#cx-rw-config` / `#cx-az-config` / `#cx-pdp-config`
present; a parse failure → `fail` with the island id, the parser message and
a 160-char excerpt around the failure position ("Liquid error" called out),
fixHint "Disarm the preview / turn the affected feature off in Markets and
re-run; send this message to support." Registered in BOTH `runHealthChecks`
arrays.

### 9. Admin

Guided plain-language page (readiness checklist, tabs Set savings / Free
gifts / Free shipping / Markets & go live, essentials first + Advanced
collapsibles, "Detect my existing KIT codes" → yieldToCodes suggestions,
new intent `detect_codes`). Banned words in UI copy: ladder, referee, node,
island, Function, alias. No replace-existing / legacy / alias UI.

### 9b. v15.1 review fixes (2026-08-17) — summary of the contract deltas

(a) `blockedCodes` (§1/§2) — server-written, sanitizer-kept, admin
read-only, never attached by storefront/checkout (best lower unblocked
tier, else no code). (b) handle-only gift options → FIRST variant by
position on both sides (§2/§3/§5). (c) `syncOk` on every preview result;
`disarmPreview` retries once (§6). (d) FEATURE_DEFS labels "Set savings" /
"Free gifts". (e) `RW = PREVIEW.rw` only when set savings or free gifts is
on/drafted (§5/§6). Merchant hardening documented in UPDATE.md: untick
"Combinations → Product discounts" on the historical KIT2..KIT10 basic
codes so Shopify itself refuses stacking with SET codes on every checkout
surface (Apple/Google Pay included — the checkout block cannot run there
and the Function cannot see other codes).

### 10. Validation pins (agent B)

harness: `#cx-rw-config` in its own tag gated live-only + closed by its own
endif + after the main island; no `"rw"` member / gifts map / all_products /
cx_gt / capture in the cart island; `cx_draft_rw` × 3; amazon rw member
live-only without giftPids; cart JS reads `#cx-rw-config` (never `cfg.rw`),
swaps in `PREVIEW.rw`, `rwYieldCodes`, no alias, `cx_rw_gifts:` cache; SET
preset codes + DEFAULT_YIELD_TO_CODES + sanitizeYieldToCodes; retired keys
gone; rewards.server has no delete/deactivate mutation names, no
replaceExisting, `isOurCodeNode`, exact foreign-code sentence,
`detectStoreCodes`, `hv`; metafields two-step + `warnings` + hv resolution;
logic.js no alias; health `storefront-islands` in both arrays + fix hints;
preview-config `rw`, `rewardsPreviewSections`, `PreviewSyncResult.warnings`;
readiness copy. settings-derivation: SET ladders, yieldToCodes
default/sanitizer/validator, retired keys, mergeSettings drops them.
rewards-function sim: stale `ss.alias` ignored, SET codes grant, KIT3 on a
SET ladder grants nothing. **v15.1 pins**: settings-derivation —
`blockedCodes` default `[]`, `REWARDS_CAPS.blockedCodes = 6`,
`sanitizeBlockedCodes` (ladder-only, upper/trim/dedupe/cap, non-array →
[]), sanitizeSettings re-derivation, Connect-style patch survives merge,
validator tolerance, FEATURE_DEFS labels; harness — rewards.server has NO
`availableForSale` / `firstAvailableVid` (hv = `firstVid`), imports
`saveSettings`, writes `rewards: { setSavings: { blockedCodes } }` before
the sync of the updated settings; disarmPreview two `resyncMetafields`
